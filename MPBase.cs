...
namespace MercadoPago
{
  public abstract class MPBase
  {
    ...
    public static List<T> ProcessMethodBulk<T>(Type clazz, string methodName, bool useCache) where T : MPBase
    {
      ...
      return MPBase.ProcessMethodBulk<T>(clazz, methodName, mapParams, useCache);
    }
    ...
    public static MPBase ProcessMethod(string methodName, bool useCache)
    {
      ...
      return MPBase.ProcessMethod<MPBase>(typeFromStack, (MPBase) null, methodName, parameters, useCache);
    }
    ...
    public MPBase ProcessMethod<T>(string methodName, bool useCache) where T : MPBase
    {
      ...
      MPBase.ProcessMethod<T>(this.GetType(), (T) this, methodName, parameters, useCache);
      return this;
    }

    public bool ProcessMethodBool<T>(string methodName, bool useCache) where T : MPBase
    {
      Dictionary<string, string> parameters = (Dictionary<string, string>) null;
      MPBase.ProcessMethod<T>(this.GetType(), (T) this, methodName, parameters, useCache);
      ...
      where T : MPBase
    {
      ...
      string path = MPBase.ParsePath<T>(restInformation["path"].ToString(), mapParams, resource);
      ...
      where T : MPBase
    {
      ...
      Dictionary<string, object> restInformation = MPBase.GetRestInformation(MPBase.GetAnnotatedMethod(clazz, methodName));
      HttpMethod httpMethod = (HttpMethod) restInformation["method"];
      string path = MPBase.ParsePath<T>(restInformation["path"].ToString(), parameters, resource);
      PayloadType payloadType = (PayloadType) restInformation["payloadType"];
